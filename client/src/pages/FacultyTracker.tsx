...
export default function FacultyTracker() {
  ...
  const getCompletionStatus = (assignments: WorkloadAssignment[]) => {
    if (assignments.length === 0) return { label: "No Tasks", variant: "secondary" as const };
    const completed = assignments.filter(a => a.status === "completed").length;
    const total = assignments.length;
    const percentage = (completed / total) * 100;
    
    if (percentage === 100) return { label: "All Complete", variant: "secondary" as const };
  ...
  return (
    <Layout title="Faculty Tracker" subtitle="View assigned tasks and completion status by faculty">
      {/* Filters */}
      <Card className="mb-6">
                          ...
                          {assignments.slice(0, 3).map((assignment) => (
                            <div key={assignment.id} className="flex items-center space-x-2 text-sm">
                              <div className={`w-2 h-2 rounded-full ${
                                ...
                                assignment.status === 'pending' ? 'bg-orange-500' : 'bg-blue-500'
                              }`}></div>
                              <span className="flex-1">{getSubjectName(assignment.subjectId)}</span>
                              <span className="text-slate-500">{assignment.hoursPerWeek}h</span>
                            </div>
                          ))}
